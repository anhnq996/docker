FROM alpine:edge
MAINTAINER Han Nguyen <ngochan.gtf@gmail.com>
# Define workdir
...
RUN if [ "$ENV" = "production" ] ; \
      ...
      then apk add --no-cache bash curl shadow sudo supervisor ; \
      else apk add --no-cache bash curl shadow sudo supervisor nodejs npm htop ; \
    fi ; \
...
RUN apk add --no-cache php81 php81-common php81-opcache php81-gd php81-pdo php81-pdo_mysql php81-zip php81-phar php81-iconv \
    ...
    php81-xml php81-xmlreader php81-xmlwriter php81-simplexml php81-dom php81-tokenizer php81-pecl-redis php81-pecl-swoole ; \
